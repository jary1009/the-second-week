...
Status TraverseLinkStack(LinkStack* e, void(*f)(char q))
{
	//思路：遍历
...
Status pushLStack1(LinkStack1* s, int data)
{
	if (s == NULL)return ERROR;
	StackNode1* temp = (StackNode1*)malloc(sizeof(StackNode1));
	if (temp == NULL)return ERROR;
	temp->data = data;
	temp->next = s->top;
...
Status popLStack1(LinkStack1* s, int* data)
{
	//思路：修改top，free掉第一个节点即可
	if (s == NULL)return ERROR;
	if (s->top == NULL)return ERROR;
