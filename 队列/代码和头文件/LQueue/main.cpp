...
//char type;//这个有用
//char datatype[30];//这个有什么用呢?害我写错了一堆函数，又没有模块化，将错就错吧
int main()
{
	get_type();
	show();
...
void get_type()
{
	while (1)
	{
		...
		//没能及时看到，以为用datatype接收了，不想在LQueue里改了，在这里小改一下吧
		scanf("%c", &type);
		while (getchar() != '\n');
...
void get_char(void** data)
{
	...
	printf("if you put in more than one character,we choose the first to add in.\n");
